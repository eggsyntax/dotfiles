...
sudo apt -y install curl vim xcape vlc xsel python-pygments openssh-server \
  ...
  net-tools chromium-browser tree sshuttle docker-ce docker-ce-cli containerd.io \
  dconf-editor graphviz gimp python-pip \
